    ...
    import { LogOut, User, LayoutDashboard, Radio, Calendar, FileText, Bot, Layers, History, List } from 'lucide-react';
    ...
    const navItems = [
      ...
      { name: 'Rádios', path: '/painel', icon: Radio },
      { name: 'Agendamentos', path: '/agendamentos', icon: Calendar },
      { name: 'Gravação em Massa', path: '/gravacao-em-massa', icon: Layers },
      { name: 'Gravações', path: '/gravacoes', icon: FileText },
      { name: 'Edição IA', path: '/edicao-ia', icon: Bot },
      { name: 'Histórico', path: '/historico', icon: History },
      { name: 'Lista', path: '/lista', icon: List },
    ];
    ...
    const Navbar = () => {
      ...
      const getNavLinkClass = (path) => {
        ...
        if (path === '/edicao-ia' && location.pathname.startsWith('/edicao-ia')) {
