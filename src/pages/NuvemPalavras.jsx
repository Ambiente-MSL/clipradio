...
const formatOffset = (value) => {
  ...
  if (!Number.isFinite(totalSeconds) || totalSeconds < 0) return 'Minutagem indisponível';
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Visualize as tags mais citadas nas transcrições, com rádio, cidade e horário de aparição."
        />
      </Helmet>
      ...
      <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(34,211,238,0.18),_transparent_34%),linear-gradient(180deg,_rgba(15,23,42,0.98)_0%,_rgba(2,6,23,1)_100%)] px-4 pb-16 pt-4 md:px-6">
        <div className="mx-auto max-w-7xl">
          <div className="rounded-[28px] border border-cyan-500/20 bg-slate-950/70 p-6 shadow-[0_24px_80px_rgba(8,145,178,0.16)] backdrop-blur-xl md:p-8">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-3xl">
                ...
                <p className="mt-4 max-w-2xl text-base leading-7 text-slate-300 md:text-lg">
                  Veja quais palavras cadastradas apareceram nas gravações transcritas, quantas vezes elas surgiram e
                  em qual rádio, cidade e horário foram ouvidas.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <div className="relative min-w-[260px]">
                  ...
                  <Input
                    ...
                    placeholder="Filtrar palavra, rádio ou cidade"
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <SummaryCard
                ...
                description={`${summary.total_tags ?? 0} tags únicas catalogadas para esta visão.`}
              ...
              <SummaryCard
                ...
                description="Total de vezes em que alguma tag apareceu nas transcrições analisadas."
                accentClass="text-emerald-300"
              />
              <SummaryCard
                icon={Radio}
                title="Gravações lidas"
                value={summary.recordings_scanned ?? 0}
                description={`${summary.recordings_with_matches ?? 0} gravações tiveram pelo menos uma correspondência.`}
                accentClass="text-amber-300"
              />
              <SummaryCard
                icon={Clock3}
                title="Ocorrências listadas"
                ...
                description={
                  summary.occurrences_truncated
                    ...
                    : 'Todas as ocorrências detalhadas retornadas pela API.'
                }
          ...
          <div className="mt-8 grid gap-6 xl:grid-cols-[1.5fr_0.9fr]">
            <Card className="border-slate-800 bg-slate-900/80 shadow-2xl shadow-black/20">
              <CardHeader className="border-b border-slate-800">
                ...
                <CardDescription className="text-slate-400">
                  O tamanho de cada palavra representa a quantidade de aparições nas gravações transcritas.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-6">
                {loading ? (
                  ...
                  <div className="flex min-h-[360px] flex-col items-center justify-center rounded-3xl border border-dashed border-slate-700 bg-slate-950/50 px-6 text-center">
                    <p className="text-lg font-semibold text-white">Nenhuma ocorrência encontrada.</p>
                    <p className="mt-2 max-w-md text-sm text-slate-400">
                      Verifique se existem transcrições concluídas e se as tags cadastradas aparecem no texto.
                    </p>
                  </div>
                ) : (
            ...
            <Card className="border-slate-800 bg-slate-900/80 shadow-2xl shadow-black/20">
              <CardHeader className="border-b border-slate-800">
                ...
                <CardDescription className="text-slate-400">
                  Rádio, cidade e horário em que a tag apareceu.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-6">
                {!selectedWord ? (
                  <div className="rounded-3xl border border-dashed border-slate-700 bg-slate-950/50 px-6 py-10 text-center">
                    ...
                    <p className="mt-2 text-sm text-slate-400">
                      Os dados detalhados de ocorrência aparecerão aqui.
                    </p>
                  </div>
                ) : (
                  <>
                    <div className="rounded-[28px] border border-slate-800 bg-slate-950/70 p-5">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="rounded-full border border-cyan-500/30 bg-cyan-500/10 px-4 py-2 text-sm font-semibold text-cyan-200">
                          {selectedWord.count} ocorrências
                        </div>
                      </div>

                      <div className="mt-5 grid gap-3 sm:grid-cols-3">
                        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-3">
                          <p className="text-xs uppercase tracking-[0.22em] text-slate-500">Gravações</p>
                          <p className="mt-2 text-2xl font-black text-white">{selectedWord.recordings_count}</p>
                        </div>
                        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-3">
                          <p className="text-xs uppercase tracking-[0.22em] text-slate-500">Rádios</p>
                    ...
                    <div className="mt-5 space-y-3">
                      {(selectedWord.sample_occurrences || []).length > 0 ? (
                        selectedWord.sample_occurrences.map((occurrence, index) => (
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <p className="font-semibold text-white">{occurrence.radio_nome || 'Rádio'}</p>
                                <div className="mt-2 flex flex-wrap gap-3 text-xs text-slate-400">
                                  <span className="inline-flex items-center gap-1">
                                    <MapPin className="h-3.5 w-3.5" />
                                    {[occurrence.cidade, occurrence.estado].filter(Boolean).join(' / ') || 'Cidade indisponível'}
                                  </span>
                                  <span className="inline-flex items-center gap-1">
                        ...
                        <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-950/55 px-4 py-8 text-center text-sm text-slate-400">
                          Nenhuma amostra detalhada disponível para esta palavra.
                        </div>
                      )}
                    </div>
                  </>
          ...
          <Card className="mt-6 border-slate-800 bg-slate-900/80 shadow-2xl shadow-black/20">
            <CardHeader className="border-b border-slate-800">
              <CardTitle className="text-white">Ocorrências detalhadas</CardTitle>
              <CardDescription className="text-slate-400">
                {selectedWord
                  ? `Mostrando onde "${selectedWord.text}" apareceu nas gravações.`
                  : 'Selecione uma palavra para filtrar as ocorrências.'}
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              ...
              {loading ? (
                ...
                <div className="px-6 py-16 text-center">
                  <p className="text-lg font-semibold text-white">Nenhuma ocorrência para exibir.</p>
                  <p className="mt-2 text-sm text-slate-400">
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          {occurrence.count > 1 ? `${occurrence.count} vezes nesta gravação` : '1 vez nesta gravação'}
                        </p>
                      </div>
                      <div className="min-w-0">
                        <p className="truncate font-medium text-slate-100">{occurrence.radio_nome || 'Rádio indisponível'}</p>
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs text-slate-400">
                          {occurrence.exact_time ? 'Horário calculado pelo timestamp da palavra.' : 'Horário aproximado pelo segmento ou indisponível.'}
                        </p>
                      </div>
